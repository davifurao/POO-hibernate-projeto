...
public class RegistroTransacaoDAO implements IEntityDAO<RegistroTransacao> {
	...
	@Override
	public RegistroTransacao findByNumeroConta(String numero) {
		Session session = conn.getSessionFactory().openSession();
	    Query<RegistroTransacao> query = session.createQuery("FROM RegistroTransacao c WHERE c.numero_conta = :numero", RegistroTransacao.class);
